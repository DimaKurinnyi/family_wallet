...
export const MonthlyFlow: React.FC<Props> = ({ months, className }) => {
  ...
  const max = Math.max(...months.flatMap((month) => [month.income, month.expense]), 0);
  ...
  return (
    <section className={cn('rounded-2xl border border-gray-100 bg-white p-4 sm:p-6', className)}>
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2">
        <h2 className="text-lg font-semibold text-gray-800">Доходы и расходы</h2>
      ...
      {max === 0 ? (
        <p className="mt-6 text-gray-400">Пока нет операций — здесь появятся столбцы по месяцам.</p>
      ) : (
        <div className="mt-4">
